...
export class PayrollCalculationEngine {
  public calculateItem(input: PayrollCalculationInput): PayrollItem {
    ...
    // No constituye salario (Art. 128 CST). Se prorratea IGUAL que el salario:
    // se divide por los días del mes y se multiplica por los días efectivamente
    // trabajados en la quincena (si no trabaja un día, ese día se descuenta).
    ...
    if (fixedBonusProrated > 0) {
      ...
      explanations.push({
        ...
        formula: `($${Math.round(employee.nonSalaryBonus || 0).toLocaleString('es-CO')} / ${periodDays} días) × ${workedDays} días laborados (No constitutivo de salario)`,
